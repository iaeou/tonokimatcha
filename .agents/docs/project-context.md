...
- The hero bead is the **illustrated Magatama** (2026-08-25), the same character as the header icon: a bevelled ink slab with the logo artwork painted on both faces, built by `createMagatamaIconGeometry()` from `magatama-icon-data.ts` (baked from `static/matchaTonoki-logo.svg`). The ink outline is not a layer — the paint is inset and the slab showing through *is* the line, so `MAGATAMA_ICON_BEVEL_SIZE` sets its weight and must change together with the data it clipped. Back paint is unrotated on purpose and relies on `DoubleSide`. `icon.envMapIntensity` is 0.25 because the jade key at 2.2 lifts 2.5%-albedo ink to grey. Selected by `MAGATAMA_TUNING.icon.enabled`; the faceted stone and smooth jade bead remain selectable behind it. See `project-status-2026-08-25b.md`.
